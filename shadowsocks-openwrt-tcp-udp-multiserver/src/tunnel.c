...
int main(int argc, char **argv)
{
...
#ifdef ANDROID
    while ((c = getopt(argc, argv, "f:s:p:l:k:t:m:i:c:b:L:a:uUvVAT")) != -1) {
#else
    while ((c = getopt(argc, argv, "f:s:p:l:k:t:m:i:c:b:L:a:uUvAT")) != -1) {
#endif
        switch (c) {
        case 's':
    ...
    listen_ctx.method = m;
    listen_ctx.dns_over_tcp = dns_over_tcp;
    ...
    if (mode != TCP_ONLY) {
        ...
        init_udprelay(local_addr, local_port, listen_ctx.remote_addr[0],
                      ...
                      &listen_ctx, tunnel_addr, m, auth, listen_ctx.timeout, iface);
    }
    ...
    if(listen_ctx.dns_over_tcp) {
        LOGI("=== TCP DNS enabled ===\n");
    }
