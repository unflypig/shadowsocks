...
static void trans_recv_cb(EV_P_ ev_io *w, int revents);
static void trans_timeout_cb(EV_P_ ev_timer *watcher, int revents);
static void close_and_free_trans(EV_P_ struct trans_ctx *ctx);
static struct trans_ctx * new_trans(int fd, struct server_ctx * server_ctx);
#endif

static char *hash_key(const int af, const struct sockaddr_storage *addr, const struct sockaddr* destaddr);
#ifdef UDPRELAY_REMOTE
...
#define HASH_KEY_LEN (sizeof(struct sockaddr_storage) + sizeof(int) + sizeof(struct sockaddr))
static char *hash_key(const int af, const struct sockaddr_storage *srcaddr, const struct sockaddr* destaddr)
{
    int offset = 0;
    static char key[HASH_KEY_LEN];
    struct sockaddr locaddr = {0};

    if(destaddr == NULL)
    {
        memset((char*)&locaddr, 0, sizeof(struct sockaddr));
        destaddr = &locaddr;
    }

    memset(key, 0, HASH_KEY_LEN);
    memcpy(key+offset, &af, sizeof(int));
    offset += sizeof(int);
    memcpy(key + offset, (const uint8_t *)srcaddr, sizeof(struct sockaddr_storage));
    offset += sizeof(struct sockaddr_storage);
    memcpy(key + offset, (const uint8_t *)destaddr, sizeof(struct sockaddr));
...
#ifdef UDPRELAY_TUNNEL
struct trans_ctx *new_trans(int fd, struct server_ctx *server_ctx)
{
    struct trans_ctx *ctx = malloc(sizeof(struct trans_ctx));
    memset(ctx, 0, sizeof(struct trans_ctx));

    ctx->fd = fd;
    ctx->server_ctx = server_ctx;
    ev_io_init(&ctx->io, trans_recv_cb, fd, EV_READ);
    ev_timer_init(&ctx->watcher, trans_timeout_cb, server_ctx->timeout,
                  server_ctx->timeout);
    return ctx;
}
#endif
...
static void remote_timeout_cb(EV_P_ ev_timer *watcher, int revents)
{
    ...
    if (verbose) {
        #ifdef UDPRELAY_TUNNEL
        LOGI("[udp] connection timeout, remote_ctx->encidx = %d", remote_ctx->encidx);
        #else
        LOGI("[udp] connection timeout");
        #endif
    }
    #ifdef UDPRELAY_TUNNEL
    char *key = hash_key(remote_ctx->af, &remote_ctx->src_addr, remote_ctx->server_ctx->multi_remote_addr[remote_ctx->encidx]);
    #else
    char *key = hash_key(remote_ctx->af, &remote_ctx->src_addr, 0);
    #endif
    cache_remove(remote_ctx->server_ctx->conn_cache, key, HASH_KEY_LEN);
}

#ifdef UDPRELAY_TUNNEL
void close_and_free_trans(EV_P_ struct trans_ctx *ctx)
{
    if (ctx != NULL) {
        ev_timer_stop(EV_A_ & ctx->watcher);
        ev_io_stop(EV_A_ & ctx->io);
...
static void trans_timeout_cb(EV_P_ ev_timer *watcher, int revents)
{
    struct trans_ctx *trans_ctx = (struct trans_ctx *)(((void *)watcher)
                                                          - sizeof(ev_io));

    if (verbose) {
        LOGI("[udp] trans connection timeout");
    }

    close_and_free_trans(EV_A_ trans_ctx);
}

static void trans_recv_cb(EV_P_ ev_io *w, int revents)
{
    struct trans_ctx *trans_ctx = (struct trans_ctx *)w;
    struct server_ctx *server_ctx = trans_ctx->server_ctx;
    ...
    if (verbose) {
        LOGI("[udp] trans receive a packet");
    }

    // recv
    ssize_t buf_len = recv(trans_ctx->fd, buf, BUF_SIZE, 0);
    if (buf_len == 0) {
        // connection closed
        goto CLEAN_UP;
    } else if (buf_len < 0) {
    ...
    if(verbose){
        int i = 0;
        char rcvbuf[BUF_SIZE] = {0};
        for(i = 0; i < buf_len; i++)
        {
            sprintf(rcvbuf + strlen(rcvbuf), "%02x,", (unsigned int)buf[i]&0xff);
        }
        debug("i = %d", i);
        debug("trans rcvbuf = %s, i = %d\n", rcvbuf, i);
    }

    size_t remote_src_addr_len = get_sockaddr_len((struct sockaddr *)&trans_ctx->src_addr);
    int s = sendto(server_ctx->fd, buf+2, buf_len-2, 0,
                   (struct sockaddr *)&trans_ctx->src_addr, remote_src_addr_len);
    if (s == -1) {
...
static void query_resolve_cb(struct sockaddr *addr, void *data)
{
    ...
    if (addr == NULL) {
        ...
        if (remote_ctx == NULL) {
            char *key = hash_key(0, &query_ctx->src_addr, 0);
        ...
        if (remote_ctx != NULL) {
            ...
            if (s == -1) {
                ...
                if (!cache_hit) {
                    // Add to conn cache
                    char *key = hash_key(0, &remote_ctx->src_addr, 0);
...
static void remote_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (verbose) {
        #ifdef UDPRELAY_TUNNEL
        LOGI("[udp] remote receive a packet, remote_ctx->encidx = %d", remote_ctx->encidx);
        #else
...
#ifdef UDPRELAY_LOCAL
#ifdef UDPRELAY_TUNNEL
    buf = ss_decrypt_all(BUF_SIZE, buf, &buf_len, server_ctx->method, 0, remote_ctx->encidx);
#else
    buf = ss_decrypt_all(BUF_SIZE, buf, &buf_len, server_ctx->method, 0, 0);
#endif
    if (buf == NULL) {
    ...
    buf = ss_encrypt_all(BUF_SIZE, buf, &buf_len, server_ctx->method, 0, 0);
#endif
...
#ifdef UDPRELAY_TUNNEL
    /*******************************
     support dns transport over tcp
     *******************************/
    if(server_ctx->dns_over_tcp == 1)
    {
        int opt = 1;
        int transfd = 0;
        int count = 0;
        ...
        struct trans_ctx* trans_ctx = NULL;
        struct sockaddr* trans_addr = server_ctx->tcp_server_addr;
        
        debug("<<<<<<<<<<< dns_over_tcp, new request >>>>>>>>>>\n");
        transfd = socket(trans_addr->sa_family, SOCK_STREAM, IPPROTO_TCP);
        if (transfd < 0) {
            ERROR("trans socket");
            goto TRANS_OUT;
        }

        setsockopt(transfd, SOL_TCP, TCP_NODELAY, &opt, sizeof(opt));
        #ifdef SO_NOSIGPIPE
        setsockopt(transfd, SOL_SOCKET, SO_NOSIGPIPE, &opt, sizeof(opt));
        #endif
        setnonblocking(transfd);

        #ifdef SET_INTERFACE
        if (server_ctx->iface) {
            setinterface(transfd, server_ctx->iface);
        }
        #endif

        connect(transfd, trans_addr, sizeof(struct sockaddr));
        sendbuf[0] = (buf_len >> 8) & 0xff;
        sendbuf[1] = buf_len & 0xff;
        memcpy(sendbuf+2, buf, buf_len);
        buflen = buf_len + 2;
        ...
        if(verbose){
            ...
            for(i = 0; i < buflen; i++)
            {
                sprintf(rcvbuf + strlen(rcvbuf), "%02x,", (unsigned int)sendbuf[i]&0xff);
            }
            debug("i = %d", i);
            debug("trans sendbuf = %s, i = %d\n", rcvbuf, i);
        }

        count = send(transfd, sendbuf, buflen, 0);
        if(count < buf_len)
        {
            debug("trans send error, buflen = %d, count = %d\n", buflen, count);
            close(transfd);
            goto TRANS_OUT;
        }

        // Init remote_ctx
        trans_ctx = new_trans(transfd, server_ctx);
        trans_ctx->src_addr = src_addr;

        // Start remote io
        ev_io_start(EV_A_ & trans_ctx->io);
        ev_timer_start(EV_A_ & trans_ctx->watcher);
    }
...
TRANS_OUT:
    ...
    buf = ss_decrypt_all(BUF_SIZE, buf, &buf_len, server_ctx->method, server_ctx->auth, 0);
    if (buf == NULL) {
        ERROR("[udp] server_ss_decrypt_all");
        goto CLEAN_UP;
    }
    ...
    char *key = hash_key(dst_addr.ss_family, &src_addr, 0);
...
#ifdef UDPRELAY_TUNNEL
    //move to loop as we have multi vps
#else
...
#ifdef UDPRELAY_TUNNEL
/* send dns udp packet to all vps */
    int index = 0;
    ...
    debug("new dns udp packet coming >>>>>>>>>>>>\n");
    for(index = 0; index < server_ctx->remote_num; index++)
    {
        debug("index = %d, remote_num = %d\n", index, server_ctx->remote_num);
        remote_addr = server_ctx->multi_remote_addr[index];
        remote_addr_len = server_ctx->remote_addr_len;

////////////////////////////////////////
        key = hash_key(ip.version == 4 ? AF_INET : AF_INET6, &src_addr, remote_addr);
        ...
        if (remote_ctx == NULL) {
            if (verbose) {
                debug("cache miss: %s:%s <-> %s:%d <-> %s", host, port,
            ...
            if (verbose) {
                debug("cache hit: %s:%s <-> %s:%d <-> %s", host, port,
                    inet_ntoa(((struct sockaddr_in *)remote_addr)->sin_addr), 
                    ntohs(((struct sockaddr_in *)remote_addr)->sin_port),
                    get_addr_str((struct sockaddr *)&src_addr));
            }
...
//////////////////////////////////////////
        if(remote_ctx == NULL) {
            remotefd = create_remote_socket(remote_addr->sa_family == AF_INET6);
            if (remotefd < 0) {
        ...
        memcpy(udpbuf, buf, buf_len);
        if (server_ctx->auth) {
            udpbuf[0] |= ONETIMEAUTH_FLAG;
        }

        udpbuf = ss_encrypt_all(BUF_SIZE, udpbuf, &buf_len, server_ctx->method, server_ctx->auth, remote_ctx->encidx);
        int s = sendto(remote_ctx->fd, udpbuf, buf_len, 0, remote_addr, remote_addr_len);
...
/* send dns udp packet to random vps */
#else 

#ifdef UDPRELAY_TUNNEL
    int index = rand() % server_ctx->remote_num;
    const struct sockaddr *remote_addr = server_ctx->multi_remote_addr[index];
#else
...
#ifdef UDPRELAY_TUNNEL
    buf = ss_encrypt_all(BUF_SIZE, buf, &buf_len, server_ctx->method, server_ctx->auth, remote_ctx->encidx);
#else
    buf = ss_encrypt_all(BUF_SIZE, buf, &buf_len, server_ctx->method, server_ctx->auth, 0);
#endif
...
#ifdef UDPRELAY_TUNNEL
                  struct listen_ctx *listen_ctx, const ss_addr_t tunnel_addr,
#endif
#endif
                  int method, int auth, int timeout, const char *iface)
{
    ...
    server_ctx->timeout = MIN_UDP_TIMEOUT;//max(timeout, MIN_UDP_TIMEOUT);
    ...
    server_ctx->multi_remote_addr = listen_ctx->remote_addr;
    server_ctx->remote_num = listen_ctx->remote_num;
    server_ctx->dns_over_tcp = listen_ctx->dns_over_tcp;

    struct sockaddr_storage *storage = malloc(sizeof(struct sockaddr_storage));
    memset(storage, 0, sizeof(struct sockaddr_storage));
    if(get_sockaddr((char*)server_host, (char*)server_port, storage, 1)){
        FATAL("failed to resolve the provided hostname");
    }
    server_ctx->tcp_server_addr = (struct sockaddr*)storage;
#endif
#endif
