...
static void server_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    *buf = ss_decrypt(BUF_SIZE, *buf, &r, server->d_ctx, 0);
    ...
    if (server->stage == 5) {
        if (server->auth && !ss_check_hash(&remote->buf, &r, server->chunk, server->d_ctx, BUF_SIZE, 0)) {
            LOGE("hash error");
            report_addr(server->fd);
            close_and_free_server(EV_A_ server);
            close_and_free_remote(EV_A_ remote);
        ...
        if (auth || (atyp & ONETIMEAUTH_FLAG)) {
            if (ss_onetimeauth_verify(server->buf + offset, server->buf, offset, server->d_ctx->evp.iv, 0)) {
        ...
        if (server->auth && !ss_check_hash(&server->buf, &server->buf_len, server->chunk, server->d_ctx, BUF_SIZE, 0)) {
            LOGE("hash error");
            report_addr(server->fd);
            close_and_free_server(EV_A_ server);
            return;
...
static void remote_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    server->buf = ss_encrypt(BUF_SIZE, server->buf, &r, server->e_ctx, 0);
...
static struct server * new_server(int fd, struct listen_ctx *listener)
{
    ...
    if (listener->method) {
        ...
        enc_ctx_init(listener->method, server->e_ctx, 1, 0);
        enc_ctx_init(listener->method, server->d_ctx, 0, 0);
    } else {
        server->e_ctx = NULL;
        server->d_ctx = NULL;
    }
...
static void free_server(struct server *server)
{
    ...
    if (server->e_ctx != NULL) {
        cipher_context_release(&server->e_ctx->evp, 0);
        free(server->e_ctx);
    }
    if (server->d_ctx != NULL) {
        cipher_context_release(&server->d_ctx->evp, 0);
        free(server->d_ctx);
    }
    if (server->buf != NULL) {
        free(server->buf);
