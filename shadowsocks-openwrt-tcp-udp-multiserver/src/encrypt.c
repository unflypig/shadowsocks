...
static uint8_t *enc_table[MAX_REMOTE_NUM];
static uint8_t *dec_table[MAX_REMOTE_NUM];
static uint8_t enc_key[MAX_REMOTE_NUM][MAX_KEY_LENGTH];
static int enc_key_len[MAX_REMOTE_NUM];
static int enc_iv_len[MAX_REMOTE_NUM];
static int enc_method[MAX_REMOTE_NUM];
...
int enc_get_iv_len(int index)
{
    return enc_iv_len[index];
}
...
void enc_table_init(const char *pass, int index)
{
    uint32_t i;
    uint64_t key = 0;
    uint8_t *digest;

    enc_table[index] = malloc(256);
    dec_table[index] = malloc(256);
    ...
    for (i = 1; i < 1024; ++i) {
        merge_sort(enc_table[index], 256, i, key);
    }
    for (i = 0; i < 256; ++i) {
        // gen decrypt table from encrypt table
        dec_table[index][enc_table[index][i]] = i;
    }
}
...
void cipher_context_init(cipher_ctx_t *ctx, int method, int enc, int index)
{
    if (method <= TABLE || method >= CIPHER_NUM) {
    ...
    if (method >= SALSA20) {
        enc_iv_len[index] = supported_ciphers_iv_size[method];
        return;
    }
    ...
    if (!EVP_CIPHER_CTX_set_key_length(evp, enc_key_len[index])) {
        EVP_CIPHER_CTX_cleanup(evp);
        LOGE("Invalid key length: %d", enc_key_len[index]);
        exit(EXIT_FAILURE);
    }
    if (method > RC4_MD5) {
                           ...
                           int enc, int index)
{
    ...
    if (enc_method[index] >= SALSA20) {
        return;
    }

    if (enc_method[index] == RC4_MD5) {
        unsigned char key_iv[32];
        memcpy(key_iv, enc_key[index], 16);
        memcpy(key_iv + 16, iv, 16);
        true_key = enc_md5(key_iv, 32, NULL);
        iv_len = 0;
    } else {
        true_key = enc_key[index];
    }
    ...
    if (cc->valid == kCCContextValid) {
        ...
        memcpy(cc->key, true_key, enc_key_len[index]);
        cc->iv_len = iv_len;
        cc->key_len = enc_key_len[index];
    ...
    if (cipher_setkey(evp, true_key, enc_key_len[index] * 8, enc) != 0) {
        cipher_free_ctx(evp);
...
#endif
#elif defined(USE_CRYPTO_MBEDTLS)
    if (mbedtls_cipher_setkey(evp, true_key, enc_key_len[index] * 8, enc) != 0) {
...
void cipher_context_release(cipher_ctx_t *ctx, int index)
{
    if (enc_method[index] >= SALSA20) {
        return;
    }
...
int ss_onetimeauth(char *auth, char *msg, int msg_len, uint8_t *iv, int index)
{
    uint8_t hash[ONETIMEAUTH_BYTES * 2];
    uint8_t auth_key[MAX_IV_LENGTH + MAX_KEY_LENGTH];
    memcpy(auth_key, iv, enc_iv_len[index]);
    memcpy(auth_key + enc_iv_len[index], enc_key[index], enc_key_len[index]);

#if defined(USE_CRYPTO_OPENSSL)
    HMAC(EVP_sha1(), auth_key, enc_iv_len[index] + enc_key_len[index], (uint8_t *)msg, msg_len, (uint8_t *)hash, NULL);
#else
    ss_sha1_hmac(auth_key, enc_iv_len[index] + enc_key_len[index], (uint8_t *)msg, msg_len, (uint8_t *)hash);
#endif
...
int ss_onetimeauth_verify(char *auth, char *msg, int msg_len, uint8_t *iv, int index)
{
    uint8_t hash[ONETIMEAUTH_BYTES * 2];
    uint8_t auth_key[MAX_IV_LENGTH + MAX_KEY_LENGTH];
    memcpy(auth_key, iv, enc_iv_len[index]);
...
#if defined(USE_CRYPTO_OPENSSL)
    HMAC(EVP_sha1(), auth_key, enc_iv_len[index] + enc_key_len[index], (uint8_t *)msg, msg_len, hash, NULL);
#else
    ss_sha1_hmac(auth_key, enc_iv_len[index] + enc_key_len[index], (uint8_t *)msg, msg_len, hash);
#endif

    return memcmp(auth, hash, ONETIMEAUTH_BYTES);
}

char * ss_encrypt_all(int buf_size, char *plaintext, ssize_t *len, int method, int auth, int index)
{
#if 0
    debug("index = %d\n", index);
#endif
    ...
    if (method > TABLE) {
        cipher_ctx_t evp;
        cipher_context_init(&evp, method, 1, index);

        size_t p_len = *len, c_len = *len;
        size_t iv_len = enc_iv_len[index];
        int err = 1;
        ...
        cipher_context_set_iv(&evp, iv, iv_len, 1, index);
        memcpy(ciphertext, iv, iv_len);

        if (auth) {
            char hash[ONETIMEAUTH_BYTES * 2];
            ss_onetimeauth(hash, plaintext, p_len, iv, index);
        ...
        if (!err) {
            free(plaintext);
            cipher_context_release(&evp, index);
            return NULL;
        }
        ...
        *len = iv_len + c_len;
        ...
        while (plaintext < begin + *len) {
            *plaintext = (char)enc_table[index][(uint8_t)*plaintext];
            plaintext++;
        }
        return begin;
    }
                  ...
                  struct enc_ctx *ctx, int index)
{
    ...
    if (ctx != NULL) {
        ...
        if (!ctx->init) {
            cipher_context_set_iv(&ctx->evp, ctx->evp.iv, iv_len, 1, index);
        ...
        if (enc_method[index] >= SALSA20) {
            ...
            crypto_stream_xor_ic((uint8_t *)(ciphertext + iv_len),
                                 ...
                                 ctx->counter / SODIUM_BLOCK_SIZE, enc_key[index],
                                 enc_method[index]);
            ctx->counter += p_len;
            if (padding) {
...
char * ss_decrypt_all(int buf_size, char *ciphertext, ssize_t *len, int method, int auth, int index)
{
#if 0
    debug("index = %d\n", index);
#endif
    ...
    if (method > TABLE) {
        cipher_ctx_t evp;
        cipher_context_init(&evp, method, 0, index);
        size_t iv_len = enc_iv_len[index];
        ...
        if (auth || (plaintext[0] & ONETIMEAUTH_FLAG)) {
            ...
            ret = !ss_onetimeauth_verify(hash, plaintext, p_len - ONETIMEAUTH_BYTES, iv, index);
        ...
        *len = p_len;
        ...
        while (ciphertext < begin + *len) {
            *ciphertext = (char)dec_table[index][(uint8_t)*ciphertext];
            ciphertext++;
        }
        return begin;
    }
                  ...
                  struct enc_ctx *ctx, int index)
{
    ...
    if (ctx != NULL) {
        ...
        if (!ctx->init) {
            ...
            p_len -= iv_len;
            memcpy(iv, ciphertext, iv_len);
            cipher_context_set_iv(&ctx->evp, iv, iv_len, 0, index);
            ctx->counter = 0;
            ctx->init = 1;
...
void enc_ctx_init(int method, struct enc_ctx *ctx, int enc, int index)
{
    memset(ctx, 0, sizeof(struct enc_ctx));
    cipher_context_init(&ctx->evp, method, enc, index);

    if (enc) {
        rand_bytes(ctx->evp.iv, enc_iv_len[index]);
    }
}

void enc_key_init(int method, const char *pass, int index)
{
    if (method <= TABLE || method >= CIPHER_NUM) {
    ...
    enc_key_len[index] = bytes_to_key(cipher, md, (const uint8_t *)pass, enc_key[index], iv);
    if (enc_key_len[index] == 0) {
        FATAL("Cannot generate key and IV");
    }
    if (method == RC4_MD5) {
        enc_iv_len[index] = 16;
    } else {
        enc_iv_len[index] = cipher_iv_size(cipher);
    }
    enc_method[index] = method;
}

int enc_get_m(const char *method)
{
    int m = TABLE;
    if (method != NULL) {
...
void enc_init(const char *pass, int midx, int servidx)
{
    if (midx == TABLE) {
        enc_table_init(pass, servidx);
    } else {
        enc_key_init(midx, pass, servidx);
    }
}

int ss_check_hash(char **buf_ptr, ssize_t *buf_len, struct chunk *chunk, struct enc_ctx *ctx, int buf_size, int index)
{
    int i, j, k;
    char *buf = *buf_ptr;
    ...
    for (i = 0, j = 0, k = 0; i < blen; i++) {
        ...
        if (cidx == chunk->len + AUTH_BYTES) {
            ...
            memcpy(key, ctx->evp.iv, enc_iv_len[index]);
            memcpy(key + enc_iv_len[index], &c, sizeof(uint32_t));
#if defined(USE_CRYPTO_OPENSSL)
            HMAC(EVP_sha1(), key, enc_iv_len[index] + sizeof(uint32_t),
                    (uint8_t *)chunk->buf + AUTH_BYTES, chunk->len, hash, NULL);
#else
            ss_sha1_hmac(key, enc_iv_len[index] + sizeof(uint32_t),
...
char *ss_gen_hash(char *buf, ssize_t *buf_len, uint32_t *counter, struct enc_ctx *ctx, int buf_size, int index)
{
    ssize_t blen = *buf_len;
    ...
    memcpy(key + enc_iv_len[index], &c, sizeof(uint32_t));
#if defined(USE_CRYPTO_OPENSSL)
    HMAC(EVP_sha1(), key, enc_iv_len[index] + sizeof(uint32_t), (uint8_t *)buf, blen, hash, NULL);
#else
    ss_sha1_hmac(key, enc_iv_len[index] + sizeof(uint32_t), (uint8_t *)buf, blen, hash);
#endif
