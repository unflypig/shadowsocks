    ...
    while (1) {
        ...
        if (server->stage == 5) {
            ...
            if (!remote->direct && remote->send_ctx->connected && auth) {
                remote->buf = ss_gen_hash(remote->buf, &r, &remote->counter, server->e_ctx, BUF_SIZE, 0);
            }

            // insert shadowsocks header
            if (!remote->direct) {
                remote->buf = ss_encrypt(BUF_SIZE, remote->buf, &r, server->e_ctx, 0);
            ...
            if (mode != TCP_ONLY && request->cmd == 3) {
                ...
                if (!remote->direct) {
                    if (auth) {
                        ...
                        ss_onetimeauth(ss_addr_to_send + addr_len, ss_addr_to_send, addr_len, server->e_ctx->evp.iv, 0);
                    ...
                    if (r > 0) {
                        if (auth) {
                            buf = ss_gen_hash(buf, &r, &remote->counter, server->e_ctx, BUF_SIZE, 0);
                        }
...
static void remote_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (!remote->direct) {
        server->buf = ss_decrypt(BUF_SIZE, server->buf, &r, server->d_ctx, 0);
        if (server->buf == NULL) {
            LOGE("invalid password or cipher");
            close_and_free_remote(EV_A_ remote);
            close_and_free_server(EV_A_ server);
...
static struct server * new_server(int fd, int method)
{
    ...
    if (method) {
        ...
        enc_ctx_init(method, server->e_ctx, 1, 0);
        enc_ctx_init(method, server->d_ctx, 0, 0);
    } else {
        server->e_ctx = NULL;
        server->d_ctx = NULL;
    }
