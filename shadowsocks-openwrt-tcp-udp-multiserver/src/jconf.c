...
void parse_maddr(const char *str, ss_addr_t *addr)
{
    char addrbuf[3][256];
    ...
    memset((char*)addrbuf, 0, sizeof(addrbuf));
    sscanf(str, "%[^:]:%[^:]:%[^:]", addrbuf[0], addrbuf[1], addrbuf[2]);

    addr->host = strdup(addrbuf[0]);
    addr->port = strdup(addrbuf[1]);
    addr->pass = strdup(addrbuf[2]);
}
...
jconf_t *read_jconf(const char * file)
{
    ...
    if (obj->type == json_object) {
        ...
        for (i = 0; i < obj->u.object.length; i++) {
            ...
            } else if (strcmp(name, "mserver") == 0) {
                if (value->type == json_string) {
                    parse_maddr(to_string(value), conf.remote_addr + conf.remote_num);
                    conf.remote_num++;
                }
